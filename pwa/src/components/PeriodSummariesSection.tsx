...
import { CalendarDays, CalendarRange, TrendingUp, Receipt, ArrowUpRight, Clock } from 'lucide-react';
            ...
            >
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h4 className="text-sm font-bold text-slate-900 dark:text-white font-heading tracking-tight">
                      {p.title}
                    </h4>
                  ...
                  <div className={`p-2.5 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 group-hover:scale-110 transition-transform duration-300`}>
                    <Icon className={`h-4.5 w-4.5 ${p.textColor}`} />
                  </div>
                </div>

                <div className="space-y-1 mb-4">
                  <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Facturación Total</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white font-heading font-mono">
                ...
                <div className="border-t border-slate-200 dark:border-slate-800/80 my-3.5" />
                ...
                <div className="space-y-2 mb-4">
                  ...
                  <div className="w-full h-2.5 bg-slate-200 dark:bg-slate-950 rounded-full overflow-hidden border border-slate-300 dark:border-slate-800 flex">
                    <div
                  ...
                  <div className="flex justify-between text-[11px] font-mono font-semibold pt-1">
                    <span className="text-violet-600 dark:text-violet-400">
                      {s.card.toLocaleString('es-ES', { minimumFractionDigits: 2 })} € ({s.pctCard.toFixed(0)}%)
                    </span>
                    <span className="text-emerald-600 dark:text-emerald-400">
                      {s.cash.toLocaleString('es-ES', { minimumFractionDigits: 2 })} € ({s.pctCash.toFixed(0)}%)
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-100/70 dark:bg-slate-950/40 p-3 rounded-xl border border-slate-200 dark:border-slate-800/60">
                  <div className="space-y-1">
                    <span className="text-[9px] text-slate-500 font-bold uppercase tracking-wider block">Coste Personal Est.</span>
                    <span className="text-[11px] font-bold text-slate-900 dark:text-white font-mono block">
                      {s.laborCost.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} €
                    </span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium font-mono flex items-center gap-0.5">
                      <Clock className="h-3 w-3 text-indigo-400 inline" />
                      {s.hours.toFixed(1)} hrs ({s.laborCostPct.toFixed(1)}%)
                    </span>
                  </div>
                  <div className="space-y-1 border-l border-slate-200 dark:border-slate-800/60 pl-3">
                    ...
                    <span className="text-[10px] text-slate-600 dark:text-slate-400 block font-mono font-medium">
                      Horas: <strong className="text-slate-900 dark:text-slate-200">{s.hours.toFixed(1)}h</strong>
                    </span>
                  </div>
                </div>
              </div>

              <div className="mt-4 pt-3.5 border-t border-slate-200 dark:border-slate-800/80 flex items-center justify-between text-xs">
